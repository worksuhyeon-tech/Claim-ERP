...
/* 조치유형 필터 — 업무 흐름 순, 교차·종결성 항목(VOC·종결취소건·종결)은 뒤로 배치.
   단계와 무관하게 항상 전체 목록을 노출한다(typesForStage 참고). */
const TYPE_ORDER = ["면허미확인", "계약조사", "선견미징구", "일보미입력", "검토미회신", "청구미수신", "손해사정완료", "상신중", "VOC", "종결취소건", "종결"];
...
/* 조치유형은 단계와 무관하게 항상 11개 전체를 노출한다(인자는 호환용으로 유지). */
function typesForStage(_stage) { return TYPE_ORDER.slice(); }
